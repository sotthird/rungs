...
[Why HTML?](https://info.arxiv.org/about/accessible_HTML.html) [Report Issue](# "Report an Issue") [Back to Abstract](/abs/2606.27457v1 "Back to abstract page") [Download PDF](/pdf/2606.27457v1 "Download PDF")[](javascript:toggleNavTOC(); "Toggle navigation")[](javascript:toggleReadingMode(); "Disable reading mode, show header and footer")

1.  [Abstract](#abstract1 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
2.  [1 Introduction](#S1 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
3.  [2 Related Work](#S2 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [LLM routing and cascades.](#S2.SS0.SSS0.Px1 "In 2 Related Work ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [Adaptive inference and quality estimation.](#S2.SS0.SSS0.Px2 "In 2 Related Work ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    3.  [Efficient LLM deployment in telecom network.](#S2.SS0.SSS0.Px3 "In 2 Related Work ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
4.  [3 System Overview](#S3 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
5.  [4 Stage 1: Clustering-Based Routing](#S4 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [4.1 Problem Formulation](#S4.SS1 "In 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [4.2 Clustering](#S4.SS2 "In 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    3.  [4.3 Pareto Analysis and Model Selection](#S4.SS3 "In 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    4.  [4.4 Crossover Points and Routing Regions](#S4.SS4 "In 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    5.  [4.5 λ\\lambda Selection](#S4.SS5 "In 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
6.  [5 Stage 2: Quality Estimation Cascade](#S5 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [5.1 QE Classifier](#S5.SS1 "In 5 Stage 2: Quality Estimation Cascade ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [5.2 Cascade Integration](#S5.SS2 "In 5 Stage 2: Quality Estimation Cascade ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
7.  [6 Datasets and Model Pool](#S6 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [AIME 2024.](#S6.SS0.SSS0.Px1 "In 6 Datasets and Model Pool ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [TeleQnA.](#S6.SS0.SSS0.Px2 "In 6 Datasets and Model Pool ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
8.  [7 Experiments and Results](#S7 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [7.1 Stage 1 – AIME 2024](#S7.SS1 "In 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [7.2 Stage 1 – TeleQnA](#S7.SS2 "In 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    3.  [7.3 Stage 1+2: AIME 2024](#S7.SS3 "In 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    4.  [7.4 Stage 1+2: TeleQnA](#S7.SS4 "In 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
9.  [8 Conclusion and Future Work](#S8 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
10.  [References](#bib "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
11.  [A Inference Setup](#A1 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [Server configuration.](#A1.SS0.SSS0.Px1 "In Appendix A Inference Setup ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [Sampling parameters.](#A1.SS0.SSS0.Px2 "In Appendix A Inference Setup ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    3.  [Model reasoning modes.](#A1.SS0.SSS0.Px3 "In Appendix A Inference Setup ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    4.  [AIME prompt suffix.](#A1.SSx1.SSS0.Px1 "In Model Prompts ‣ Appendix A Inference Setup ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    5.  [TeleQnA prompt suffix.](#A1.SSx1.SSS0.Px2 "In Model Prompts ‣ Appendix A Inference Setup ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
12.  [B Framework Extensibility: AIME Pool Expansion](#A2 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [Dominated model (Qwen3-4B).](#A2.SS0.SSS0.Px1 "In Appendix B Framework Extensibility: AIME Pool Expansion ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [Pareto-superior model (Qwen3.5-35B-A3B-FP8).](#A2.SS0.SSS0.Px2 "In Appendix B Framework Extensibility: AIME Pool Expansion ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
13.  [C QE Cascade Classifiers](#A3 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [Input format.](#A3.SS0.SSS0.Px1 "In Appendix C QE Cascade Classifiers ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [Label construction.](#A3.SS0.SSS0.Px2 "In Appendix C QE Cascade Classifiers ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    3.  [Training data.](#A3.SS0.SSS0.Px3 "In Appendix C QE Cascade Classifiers ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    4.  [Hyperparameters.](#A3.SS0.SSS0.Px4 "In Appendix C QE Cascade Classifiers ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
14.  [D QE Classifier Ablation (AIME)](#A4 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
15.  [E QE Cascade Per-Run Results](#A5 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [E.1 AIME](#A5.SS1 "In Appendix E QE Cascade Per-Run Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [E.2 TeleQnA](#A5.SS2 "In Appendix E QE Cascade Per-Run Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
16.  [F Routing and Cascading Overhead Analysis](#A6 "In Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    1.  [Clustering and routing overhead.](#A6.SS0.SSS0.Px1 "In Appendix F Routing and Cascading Overhead Analysis ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")
    2.  [QE classifier overhead.](#A6.SS0.SSS0.Px2 "In Appendix F Routing and Cascading Overhead Analysis ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")

[License: CC BY 4.0](https://info.arxiv.org/help/license/index.html#licenses-available)

arXiv:2606.27457v1 \[cs.PF\] 25 Jun 2026

Cluster, Route, Escalate:  
Cascaded Framework for Cost-Aware LLM Serving
=========================================================================

Yasmin Moslem1∗   Magdalena Kacmajor1  
Vasudevan Nedumpozhimana1 Ammar Abbas1   Solmaz Panahi1  
David Lynch2   Zhuangzhuang Nie2   Alexandros Agapitos2   Aleksandar Milenovic2  
Hongmeng Song2   Yucheng Shi2   Yue Pan2   Patricia Buffini1   John D. Kelleher1∗  
...
Yasmin Moslem1∗   Magdalena Kacmajor1 Vasudevan Nedumpozhimana1 Ammar Abbas1   Solmaz Panahi1 David Lynch2   Zhuangzhuang Nie2   Alexandros Agapitos2   Aleksandar Milenovic2 Hongmeng Song2   Yucheng Shi2   Yue Pan2   Patricia Buffini1   John D. Kelleher1∗ 1ADAPT Centre, Trinity College Dublin 2Huawei Research ∗ yasmin.moslem, john.kelleher {at} adaptcentre.ie
...
Model routing addresses this by directing each query to the most appropriate model in a pool, but existing systems typically require annotations beyond standard task evaluation. Jointly optimising routing under an explicit inference cost budget and recovering accuracy through post-generation quality estimation, using only task-correctness labels, remains underexplored (Moslem and Kelleher, [2026](#bib.bib115 "Dynamic model routing and cascading for efficient LLM inference: A survey")).

QueryClusteringK-meansRouting DecisionScore = Error + λ⋅\\lambda\\cdot CostEfficient Model 1Efficient Model 2Strong ModelQECheckQECheckFinal Output low costhigh accuracyrejectrejectacceptaccept
...
*   •
    ...
    *   (i)
        ...
        Semantic clustering: kk\-means partitions queries by semantic similarity into coherent groups.
        
    *   (ii)
        
        Cost-aware routing: each model is scored by its cost-adjusted error per cluster via a single hyperparameter λ\\lambda.
        
    *   (iii)
        
        Automatic λ\\lambda selection: λ∗\\lambda^{\*} is automatically tuned on training data to satisfy a user-specified TPOT budget BB, and applied unchanged at test time.
...
We evaluate on TeleQnA (Maatouk et al., [2025](#bib.bib111 "TeleQnA: A Benchmark Dataset to Assess Large Language Models Telecommunications Knowledge")) (telecommunications QA) and AIME 2024 (mathematical reasoning) using pools from the Qwen 3, Qwen 3.5, and Gemma 4 families, confirming generalisation across domains: Stage 1+2 retains 97% of the strongest model’s accuracy on TeleQnA; on AIME 2024, it retains nearly all of the strongest model’s accuracy at 18% lower TPOT.
...
While model routing makes a single decision to map the query to one model, model cascading operates sequentially, escalating to larger models when the initial response is insufficient (Moslem and Kelleher, [2026](#bib.bib115 "Dynamic model routing and cascading for efficient LLM inference: A survey")). HybridLLM (Ding et al., [2024](#bib.bib18 "Hybrid LLM: Cost-Efficient and Quality-Aware Query Routing")) trains a binary router to direct queries to a small or large model by jointly optimising cost and quality. RouteLLM (Ong et al., [2025](#bib.bib19 "RouteLLM: Learning to Route LLMs with Preference Data")) learns a routing policy from human preference data; its routers include matrix factorisation and causal LLM classifiers trained on Chatbot Arena data (Chiang et al., [2024](#bib.bib33 "Chatbot Arena: an open platform for evaluating LLMs by human preference")). IRT-Router (Song et al., [2025](#bib.bib106 "IRT-router: Effective and interpretable multi-LLM routing via item response theory")) applies item response theory to model query difficulty and LLM ability jointly, combining predicted correctness probability with cost into a routing score for interpretable decisions. UniRoute (Jitkrittum et al., [2026](#bib.bib44 "Universal model routing for efficient LLM inference")) addresses dynamic routing where previously unseen LLMs become available at test time, representing each model as a feature vector derived from its predictions on a set of representative prompts. Our framework also adapts to model pool changes, but differs in two respects: we automatically select λ∗\\lambda^{\*} to satisfy an explicit TPOT budget rather than sweeping λ\\lambda over a monetary-cost trade-off curve, and add a QE cascade for post-routing accuracy recovery.

FrugalGPT (Chen et al., [2024](#bib.bib7 "FrugalGPT: How to Use Large Language Models While Reducing Cost and Improving Performance")) introduced cascade-based LLM serving that queries a sequence of models adaptively, stopping when a quality threshold is reached. AutoMix (Aggarwal et al., [2024](#bib.bib8 "AutoMix: Automatically Mixing Language Models")) uses self-verification to decide escalation. Firewall routing (Peng et al., [2025](#bib.bib45 "Firewall routing: Blocking leads to better hybrid inference for LLMs")) blocks queries from reaching small models when predicted failure rates are too high. While we share the goal of cascaded LLM serving, our framework differs in two ways: Stage 1 pre-routes hard queries directly to the strong model under an explicit TPOT budget, avoiding wasted efficient-model calls, while Stage 2 uses a separate lightweight classifier trained on task-correctness labels as a QE cascade.

#### Adaptive inference and quality estimation.

Estimating output quality without a reference signal enables adaptive decisions at inference time. Self-REF (Chuang et al., [2025a](#bib.bib40 "Learning to Route LLMs with Confidence Tokens")) trains LLMs to emit confidence tokens that trigger escalation, while Chuang et al. ([2025b](#bib.bib46 "Confident or seek stronger: Exploring uncertainty-based on-device LLM routing from benchmarking to generalization")) benchmark uncertainty-driven routing from on-device SLMs to stronger LLMs. CP-Router (Su et al., [2025](#bib.bib47 "CP-Router: An uncertainty-aware Router between LLM and LRM")) applies conformal prediction to route between standard LLMs and large reasoning models based on output uncertainty. Farinhas et al. ([2025](#bib.bib103 "Translate smart, not hard: Cascaded translation systems with quality-aware deferral")) propose quality-aware deferral in a cascaded translation system, escalating outputs when a quality estimator predicts failure; our Stage 2 applies the same deferral principle across domains using task-correctness supervision. Adaptive thinking-length control (Zhang et al., [2025a](#bib.bib22 "AdaptThink: Reasoning models can learn when to think"), [b](#bib.bib41 "When to continue thinking: Adaptive thinking mode switching for efficient reasoning")) adjusts compute per query at the level of reasoning steps rather than model selection. Speculative decoding (Leviathan et al., [2023](#bib.bib599 "Fast inference from Transformers via speculative decoding")) accelerates generation within a single model, while our framework routes and escalates across models.

#### Efficient LLM deployment in telecom network.

Telecommunications is a demanding setting for efficient LLM inference, where domain competence and serving efficiency are both first-order. TeleQnA shows that general-purpose LLMs struggle with standards questions, motivating telecom-specialised models (Maatouk et al., [2025](#bib.bib111 "TeleQnA: A Benchmark Dataset to Assess Large Language Models Telecommunications Knowledge")). The field is shifting from human-in-the-loop co-pilots toward autonomous multi-agent systems owning the full lifecycle, from detection and diagnosis to remediation and validation (Xiao et al., [2026](#bib.bib626 "TeleCom-Bench: How Far Are Large Language Models from Industrial Telecommunication Applications?"); NVIDIA, [2026](#bib.bib627 "NVIDIA Advances Autonomous Networks With Agentic AI Blueprints and Telco Reasoning Models")). These always-on agents reshape the serving problem; since they act on sensitive operational data, their models must run on-premise, avoiding external APIs to preserve data privacy and sovereignty. High-volume concurrent events and long-horizon workflows make per-token latency a major constraint. Hence, our framework optimises for TPOT, where low-TPOT models keep such autonomous loops responsive on fixed compute. Routing with cascaded escalation fits naturally, as small or domain-adapted models can handle routine sub-tasks, while stronger reasoning models are reserved for more challenging cases. Rather than commissioning a new telecom-specialised model, such as the roughly 30B Nemotron-based model fine-tuned on telecom data (NVIDIA, [2026](#bib.bib627 "NVIDIA Advances Autonomous Networks With Agentic AI Blueprints and Telco Reasoning Models"); AdaptKey, [2026](#bib.bib629 "AdaptKey-nemotron-30b: a telecom-specialized language model")), our framework treats the on-premise pool as a deployment substrate. It can reuse specialised models where they suffice (e.g. VibeThinker-1.5B (Xu et al., [2025](#bib.bib35 "Tiny model, big logic: Diversity-driven optimization elicits large-model reasoning ability in VibeThinker-1.5B")) trained for maths and coding) and absorb new ones through Pareto analysis and updated routing tables, rather than expensive specialisation cycles. Trained only from task-correctness labels, it gives operators an interpretable knob trading accuracy against inference cost.

3 System Overview
-----------------

Figure [1](#S1.F1 "Figure 1 ‣ 1 Introduction ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") illustrates the full two-stage system. An incoming query is encoded and assigned to a cluster (Stage 1), then routed to one of several candidate models based on a decision score. Outputs from efficient models are passed through the QE classifier (Stage 2): accepted outputs are returned to the user; queries with low-quality outputs are re-routed to a stronger model.

Cluster centroids and per-cluster routing tables are computed once offline from task-correctness labels on the training corpus, the only annotation required, which is available from standard benchmark evaluation. At inference, each query undergoes a single embedding lookup and one arg​min\\operatorname\*{arg\\,min} over the model pool. Pool updates require only inference on the existing training corpus, with no additional annotation.

Without Stage 1, the QE cascade would need to run an efficient model on every query before deciding to escalate. Stage 1 avoids this by routing entire clusters directly to a strong model where warranted, reserving Stage 2 for per-query failures within clusters assigned to an efficient model. Together, the two stages span the routing design space from pre-generation query routing to post-generation response evaluation, addressing the multi-stage cascade gap identified in prior work (Moslem and Kelleher, [2026](#bib.bib115 "Dynamic model routing and cascading for efficient LLM inference: A survey")).
...
Let ℳ\={m1,…,mK}\\mathcal{M}=\\{m\_{1},\\ldots,m\_{K}\\} be a pool of candidate models and 𝒞\={c1,…,cN}\\mathcal{C}=\\{c\_{1},\\ldots,c\_{N}\\} a partition of the query space into NN clusters. For each model mm and cluster cc, let Error​(m,c)\\mathrm{Error}(m,c) denote the empirical error rate on training queries in that cluster. The routing score is:

Score​(m,c)\=Error​(m,c)+λ⋅Costnorm​(m)\\mathrm{Score}(m,c)=\\mathrm{Error}(m,c)+\\lambda\\cdot\\mathrm{Cost}\_{\\mathrm{norm}}(m)

(1)

where λ≥0\\lambda\\geq 0 controls the accuracy-latency trade-off and Costnorm​(m)\\mathrm{Cost}\_{\\mathrm{norm}}(m) normalises our primary efficiency metric, Time Per Output Token (TPOT), over the pool:

Costnorm​(m)\=TPOT​(m)−TPOTminTPOTmax−TPOTmin\\mathrm{Cost}\_{\\mathrm{norm}}(m)=\\frac{\\mathrm{TPOT}(m)-\\mathrm{TPOT}\_{\\mathrm{min}}}{\\mathrm{TPOT}\_{\\mathrm{max}}-\\mathrm{TPOT}\_{\\mathrm{min}}}

(2)

The fastest model has Costnorm\=0\\mathrm{Cost}\_{\\mathrm{norm}}{=}0 and the slowest has Costnorm\=1\\mathrm{Cost}\_{\\mathrm{norm}}{=}1, making λ\\lambda directly interpretable as the maximum tolerated error-rate penalty for using the most expensive model. Each query is routed to arg​minm⁡Score​(m,c)\\operatorname\*{arg\\,min}\_{m}\\mathrm{Score}(m,c), with ties broken in favour of the faster model.

### 4.2 Clustering

We adopt the query-embedding clustering scheme of Jitkrittum et al. ([2026](#bib.bib44 "Universal model routing for efficient LLM inference")): queries are encoded with all-MiniLM-L6-v2 (Wang et al., [2020](#bib.bib510 "MINILM: deep self-attention distillation for task-agnostic compression of pre-trained transformers")) and then clustered using kk\-means. The cluster count NN is selected by maximising the mean Silhouette score (Rousseeuw, [1987](#bib.bib112 "Silhouettes: A graphical aid to the interpretation and validation of cluster analysis")) over k∈\[2,10\]k\\in\[2,10\]. Centroids are fixed on training data; at inference each new query is assigned to the nearest centroid in a single embedding pass.

### 4.3 Pareto Analysis and Model Selection

A model mm is Pareto-dominated if there exists m′m^{\\prime} such that TPOT​(m′)≤TPOT​(m)\\mathrm{TPOT}(m^{\\prime})\\leq\\mathrm{TPOT}(m) and Error​(m′,c)≤Error​(m,c)\\mathrm{Error}(m^{\\prime},c)\\leq\\mathrm{Error}(m,c) for all cc, with at least one strict inequality. Dominated models cannot be selected by Equation [1](#S4.E1 "In 4.1 Problem Formulation ‣ 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") for any λ\\lambda and are discarded. When new models arrive, Pareto analysis is re-run automatically. On TeleQnA (Section [7.2](#S7.SS2 "7.2 Stage 1 – TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")), this pruning reduces the four-model pool to two Pareto-efficient candidates.

### 4.4 Crossover Points and Routing Regions

For any pool of KK models and NN clusters, sweeping λ\\lambda from 0 upwards produces routing-region boundaries where the arg​minm\\operatorname\*{arg\\,min}\_{m} assignment changes for at least one cluster; between boundaries a fixed routing strategy holds. For K\>2K{>}2, these boundaries are identified numerically via the arg​minm\\operatorname\*{arg\\,min}\_{m} sweep; this applies, for instance, to TeleQnA’s four-model pool (Section [7.2](#S7.SS2 "7.2 Stage 1 – TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")). For K\=2K{=}2, the boundary for each cluster has the closed form:

λc\=Error​(mfast,c)−Error​(mstrong,c)\\lambda\_{c}=\\mathrm{Error}(m\_{\\mathrm{fast}},c)-\\mathrm{Error}(m\_{\\mathrm{strong}},c)

(3)

This simplification holds since Costnorm​(mfast)\=0\\mathrm{Cost}\_{\\mathrm{norm}}(m\_{\\mathrm{fast}}){=}0 and Costnorm​(mstrong)\=1\\mathrm{Cost}\_{\\mathrm{norm}}(m\_{\\mathrm{strong}}){=}1, making the cost-difference denominator equal to 1. For λ<λc\\lambda<\\lambda\_{c} the larger model is preferred (lower penalty on inference cost); above it the efficient model is preferred (higher penalty on inference cost). With NN clusters, the NN crossover points define N+1N{+}1 fully interpretable routing regions; all λ\\lambda values within a region yield the same cluster-to-model assignment. Figure [2](#S4.F2 "Figure 2 ‣ 4.4 Crossover Points and Routing Regions ‣ 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") illustrates the four regions for the AIME 2024 two-model pool.

λ\\lambda00.0520.0520.0670.0670.0990.099C1↑\\!\\uparrowC0↑\\!\\uparrowC2↑\\!\\uparrowR1All Q3-30BR2C0: Q3-30BC1: VC2: Q3-30BR3C0: VC1: VC2: Q3-30BR4All V

Figure 2: Routing regions for the AIME 2024 two-model pool. As the inference cost penalty λ\\lambda increases, clusters flip from Qwen3-30B-A3B-Thinking-2507-FP8 (Q3-30B) to VibeThinker-1.5B (V) at the indicated crossover points; C1 flips first (smallest Q3-30B advantage) and C2 last (largest).

### 4.5 λ\\lambda Selection

Given a cost budget BB, the optimal λ\\lambda is:

λ∗\=arg​maxλ⁡{Acc​(λ)∣TPOT​(λ)≤B}\\lambda^{\*}=\\operatorname\*{arg\\,max}\_{\\lambda}\\bigl\\{\\mathrm{Acc}(\\lambda)\\mid\\mathrm{TPOT}(\\lambda)\\leq B\\bigr\\}

(4)

where Acc​(λ)\\mathrm{Acc}(\\lambda) and TPOT​(λ)\\mathrm{TPOT}(\\lambda) denote system accuracy and average TPOT under routing strategy λ\\lambda, selected on training data and applied unchanged at test time. BB is a user-specified deployment target, where we adopt B\=20B{=}20 ms in the experiments. We define an efficiency metric quantifying accuracy cost per millisecond saved relative to the λ\=0\\lambda{=}0 baseline (routing all queries to the strong model):

η​(λ)\=Acc​(0)−Acc​(λ)TPOT​(0)−TPOT​(λ)\\eta(\\lambda)=\\frac{\\mathrm{Acc}(0)-\\mathrm{Acc}(\\lambda)}{\\mathrm{TPOT}(0)-\\mathrm{TPOT}(\\lambda)}

(5)

Lower η\\eta indicates a more favourable trade-off.
...
We fine-tune ModernBERT-base (Warner et al., [2025](#bib.bib67 "Smarter, better, faster, longer: A modern bidirectional encoder for fast, memory efficient, and long context finetuning and inference")) as a binary classifier with labels _accept_ and _escalate_. The input combines the query, the model output, and the generation length; training labels are derived from task-correctness of the efficient model’s output. Dataset-specific input formats, training data, and hyperparameters are detailed in Appendix [C](#A3 "Appendix C QE Cascade Classifiers ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving"). At inference, the arg​max\\operatorname\*{arg\\,max} over class probabilities determines acceptance or escalation to a stronger model.

### 5.2 Cascade Integration

Stage 2 is applied only to outputs from efficient models. Outputs already produced by a strong model bypass the classifier, preserving Stage 1 latency savings. Relative to generation, the classifier adds only a small per-token cost, quantified in Appendix [F](#A6 "Appendix F Routing and Cascading Overhead Analysis ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving").
...
The American Invitational Mathematics Examination dataset provides 921 training queries (AIME 1983–2023) and 30 test queries (AIME 2024), the full fixed competition set for this benchmark. Silhouette analysis selects 3 clusters. The primary model pool consists of VibeThinker-1.5B (hereafter V) (Xu et al., [2025](#bib.bib35 "Tiny model, big logic: Diversity-driven optimization elicits large-model reasoning ability in VibeThinker-1.5B")), trained for maths and coding, and Qwen3-30B-A3B-Thinking-2507-FP8 (hereafter Q3-30B) (Yang et al., [2025](#bib.bib113 "Qwen3 Technical Report")). In the extensibility experiment (Appendix [B](#A2 "Appendix B Framework Extensibility: AIME Pool Expansion ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")), we further consider Qwen3-4B-Thinking-2507-FP8 and Qwen3.5-35B-A3B-FP8 (Yang et al., [2025](#bib.bib113 "Qwen3 Technical Report")).

#### TeleQnA.

TeleQnA (Maatouk et al., [2025](#bib.bib111 "TeleQnA: A Benchmark Dataset to Assess Large Language Models Telecommunications Knowledge")) is a multiple-choice QA benchmark for the telecommunications domain. We use 9,000 training queries and 1,000 test queries; Silhouette analysis identifies 2 clusters. The initial model pool contains Qwen3-4B-Instruct (Yang et al., [2025](#bib.bib113 "Qwen3 Technical Report")) (Q3-4B), Gemma4-E2B-it (G-E2B), Gemma4-26B-it (G-26B), and Gemma4-E4B-it (Kamath et al., [2025](#bib.bib114 "Gemma 3 Technical Report")) (G-E4B).
...
VibeThinker (V) is a fast model trained for maths and coding but still less accurate than Qwen3-30B-A3B (Q3-30B) which is larger and slower. Per-cluster training performance is shown in Table [1](#S7.T1 "Table 1 ‣ 7.1 Stage 1 – AIME 2024 ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving"). The three crossover points (Equation [3](#S4.E3 "In 4.4 Crossover Points and Routing Regions ‣ 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")) are λ0\=0.067\\lambda\_{0}{=}0.067, λ1\=0.052\\lambda\_{1}{=}0.052, λ2\=0.099\\lambda\_{2}{=}0.099, defining four routing regions (cf. Figure [2](#S4.F2 "Figure 2 ‣ 4.4 Crossover Points and Routing Regions ‣ 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") and Table [2](#S7.T2 "Table 2 ‣ 7.1 Stage 1 – AIME 2024 ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")).
...
Table [2](#S7.T2 "Table 2 ‣ 7.1 Stage 1 – AIME 2024 ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") shows routing performance on the training data, with one representative λ\\lambda per routing region. With a TPOT budget B\=20B{=}20 ms, Equation [4](#S4.E4 "In 4.5 𝜆 Selection ‣ 4 Stage 1: Clustering-Based Routing ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") selects λ∗\=0.06\\lambda^{\*}{=}0.06: C1, where Q3-30B’s accuracy advantage over V is smallest (crossover point λ1\=0.052\\lambda\_{1}{=}0.052), is assigned to V, and C0 and C2 to Q3-30B. Concretely, at λ\=0.06\\lambda{=}0.06, V scores 0.0830.083 on C1 versus Q3-30B’s 0.0910.091, while Q3-30B wins C0 (0.1230.123 vs. 0.1300.130) and C2 (0.1430.143 vs. 0.1820.182).
...
Table 2: Routing strategies on the AIME training set for selected λ\\lambda values. C0/C1/C2 indicate the model assigned to each cluster.

Table [3](#S7.T3 "Table 3 ‣ 7.1 Stage 1 – AIME 2024 ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") reports routing behavior across λ\\lambda values on the AIME 2024 test set. Performance closely mirrors training trends, validating λ\\lambda selection on training data. At λ\=0.06\\lambda{=}0.06, TPOT is reduced by 19% (from 11.8 to 9.5 ms) at a cost of 2.7% accuracy (89.1%→86.4%89.1\\%\\to 86.4\\%). Full strategy comparisons including the combined Stage 1+2 system are in Table [7](#S7.T7 "Table 7 ‣ 7.3 Stage 1+2: AIME 2024 ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving").
...
101012121414161618182020222224242626282830303232585860606262646466666868707072727474767678788080Q-4BG-26BG-E2BG-E4BTPOT (ms)Accuracy (%)DominatedPareto-efficientPareto frontier
...
Table 5: Routing strategies on the TeleQnA training set for selected λ\\lambda values.
...
Table 6: Routing assignments by λ\\lambda on the TeleQnA test set.

44668810101212141476768080848488889292TPOT (ms)Accuracy (%)Pareto frontierBaselines (V / Q3-30B)Stage 1 (λ\=0.06\\lambda{=}0.06)Stage 1+2 (λ\=0.06\\lambda{=}0.06)

(a) AIME 2024: Stage 1+2 achieves 88.4% at 9.7 ms, within 0.7 pp of always using Q3-30B at 18% lower latency.

13131616191922222525636366666969727275757878TPOT (ms)Accuracy (%)Pareto frontierBaselines (Q3-4B / G-26B)Stage 1 (λ\=0.07\\lambda{=}0.07)Stage 1+2 (λ\=0.07\\lambda{=}0.07)
...
Table [5](#S7.T5 "Table 5 ‣ 7.2 Stage 1 – TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") shows training-set routing performance, with one representative λ\\lambda per routing region. With budget B\=20B{=}20 ms, λ∗\=0.07\\lambda^{\*}{=}0.07, assigning C0 to Q3-4B and C1 to G-26B (η\=0.63​pp/ms\\eta=0.63\\,\\,\\mathrm{pp/ms}). Concretely, at λ\=0.07\\lambda{=}0.07, Q3-4B scores 0.2970.297 on C0 versus G-26B’s 0.3010.301, while G-26B wins C1 (0.3240.324 vs. 0.3290.329). Table [6](#S7.T6 "Table 6 ‣ 7.2 Stage 1 – TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") reports routing behaviour across λ\\lambda values on the TeleQnA test set; performance mirrors training trends, validating λ\\lambda selection on training data. Full strategy comparisons including the combined Stage 1+2 system are in Table [8](#S7.T8 "Table 8 ‣ 7.4 Stage 1+2: TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving").

### 7.3 Stage 1+2: AIME 2024

At λ\=0.06\\lambda{=}0.06, Stage 1 routes C1 (10 of 30 test queries) to VibeThinker; C0 and C2 go directly to Q3-30B and bypass the QE classifier.
...
Stage 1 only (λ\=0.06\\lambda{=}0.06, Q3-30B/V/Q3-30B)

86.4%

9.5

Stage 1 + 2 (λ\=0.06\\lambda{=}0.06, QE on C1)
...
At λ\=0.07\\lambda{=}0.07, Stage 1 routes C0 (590 of 1,000 test queries) to Q3-4B; C1 goes directly to G-26B and bypasses the QE classifier.

The classifier escalates on average 202 of 590 C0 queries per run from Q3-4B to G-26B, raising C0 accuracy from 68.9% to 74.0%. The overall system achieves 74.3% accuracy at 23.8 ms TPOT. Table [8](#S7.T8 "Table 8 ‣ 7.4 Stage 1+2: TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") and Figure [4(b)](#S7.F4.sf2 "In Figure 4 ‣ 7.2 Stage 1 – TeleQnA ‣ 7 Experiments and Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") compare all systems on the TeleQnA test set.
...
Stage 1 (λ\=0.07\\lambda{=}0.07, Q3-4B/G-26B)

71.2%

19.1

Stage 1 + 2 (λ\=0.07\\lambda{=}0.07, QE on C0)
...
The current framework generalises to any model pool size; hence, scaling to larger model pools is a natural next step. In a multi-model cascade the λ\\lambda table provides a natural escalation order, since decreasing λ\\lambda corresponds to increasing model capability. Future work also includes extending the QE cascade to multi-class decisions (accept, escalate, or request additional reasoning), online adaptation of λ\\lambda to query distribution shift, and incorporating additional efficiency metrics.
...
Sophisticated chain-of-thought reasoning primarily improves performance on complex maths and logic tasks, with limited gains for other tasks (Wei et al., [2022](#bib.bib192 "Chain-of-Thought Prompting Elicits Reasoning in Large Language Models"); Sprague et al., [2025](#bib.bib71 "To CoT or not to CoT? Chain-of-thought helps mainly on math and symbolic reasoning")). For the AIME dataset, VibeThinker-1.5B and Qwen3-30B-A3B generate explicit chain-of-thought reasoning wrapped in <think> tags. For the TeleQnA dataset, Qwen3-4B-Instruct produces direct answers without think tags, and Gemma4-26B-it has thinking disabled. For both models, we encourage generation of an “explanation” before generating the numerical answer. When we tried to enable the reasoning mode for Gemma4-26B-it for the TeleQnA dataset, the accuracy gain was minimal (≈\\approx3 pp) compared to just asking for the brief explanation; however, the output was much longer with explicit reasoning, which resulted in several times slower request throughput. Hence, we concluded that explicitly enabling reasoning is both unbeneficial and inefficient for the TeleQnA dataset.
...
Explanation: <one\_sentence\_explanation>
Answer: <choice\_number\_only>
0. F1 interface
1. E2 interface
2. CPRI interface
3. O1 interface
...
Table [9](#A2.T9 "Table 9 ‣ Pareto-superior model (Qwen3.5-35B-A3B-FP8). ‣ Appendix B Framework Extensibility: AIME Pool Expansion ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") shows per-model statistics for the extensibility experiment on AIME 2024.

#### Dominated model (Qwen3-4B).

Qwen3-4B-Thinking-2507-FP8 (avg. TPOT 24.99 ms, Costnorm\=1.019\\mathrm{Cost}\_{\\mathrm{norm}}{=}1.019) has strictly higher error rates than Q3-30B in all clusters. It is Pareto-dominated and never selected for any λ\\lambda.

#### Pareto-superior model (Qwen3.5-35B-A3B-FP8).

Q3.5 (avg. TPOT 17.24 ms, Costnorm\=0.52\\mathrm{Cost}\_{\\mathrm{norm}}{=}0.52) achieves lower error rates than both V and Q3-30B in all clusters; at λ\=0.06\\lambda{=}0.06 it wins all three. Table [10](#A2.T10 "Table 10 ‣ Pareto-superior model (Qwen3.5-35B-A3B-FP8). ‣ Appendix B Framework Extensibility: AIME Pool Expansion ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") shows a Pareto improvement over the two-model configuration: 89.3% accuracy at 11.0 ms vs. 86.4% at 9.5 ms, and a small improvement over always using Q3-30B (89.1% at 11.8 ms).

Model

𝐂𝐨𝐬𝐭𝐧𝐨𝐫𝐦\\boldsymbol{\\mathrm{Cost}\_{\\mathrm{norm}}}
...
∗Pareto-dominated: never selected for any λ\\lambda.

Table 9: Per-model statistics for the AIME extensibility experiment (training data). Costnorm\\mathrm{Cost}\_{\\mathrm{norm}} normalises TPOT over the two-model base pool (V and Q3-30B). Q3-4B has Costnorm\>1\\mathrm{Cost}\_{\\mathrm{norm}}{>}1 and higher per-cluster error than Q3-30B in all clusters, making it Pareto-dominated.
...
2-model λ\=0.06\\lambda{=}0.06 (Q3-30B/V/Q3-30B)
...
3-model λ\=0.06\\lambda{=}0.06 (Q3.5×\\times3)
...
We fine-tune ModernBERT-base (Warner et al., [2025](#bib.bib67 "Smarter, better, faster, longer: A modern bidirectional encoder for fast, memory efficient, and long context finetuning and inference")) as a binary sequence classifier with labels Accept (the efficient model’s answer is correct) and Route (escalate to the strong model).

#### Input format.

For AIME, the classifier receives the concatenation \[query\] \[SEP\] \[model\_output\] \[SEP\] \[num\_output\_tokens\], where the model output is truncated to the last 1,000 words. Including the output-length token makes the chain-of-thought length (a proxy for model confidence) directly available to the classifier. TeleQnA model outputs are considerably shorter (average 29 words / 40 tokens on the test set, median 28 words / 38 tokens); no truncation is applied.
...
For AIME, training examples are drawn from AIME 1983–2023 (921 queries), with model outputs from both VibeThinker-1.5B and Qwen3-4B-Instruct (1 run each), yielding 921×2\=1,842921\\times 2=1{,}842 examples. For TeleQnA, training examples are drawn from 9,000 queries with outputs from Qwen3-4B-Instruct across 5 runs (9,000×5\=45,0009{,}000\\times 5=45{,}000 examples).
...
Table [13](#A5.T13 "Table 13 ‣ E.2 TeleQnA ‣ Appendix E QE Cascade Per-Run Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving") reports per-run statistics for the QE cascade on TeleQnA C0 (590 queries). C0 accuracy ranges from 73.4% to 74.5% across runs (spread: 1.1 pp), confirming the stability of the classifier. The false discovery rate (the fraction of escalations that are correct Q3-4B answers escalated unnecessarily) averages 104/202.4 ≈\\approx 51%, indicating that the classifier errs on the side of caution. The TPOT cost of these escalations is partly offset by the accuracy recovered after roughly 98 true-positive detections per run (202.4−104202.4-104). The classifier additionally accepts on average 85 incorrect answers per run (false negatives, Table [13](#A5.T13 "Table 13 ‣ E.2 TeleQnA ‣ Appendix E QE Cascade Per-Run Results ‣ Cluster, Route, Escalate: Cascaded Framework for Cost-Aware LLM Serving")). Approaches to improving classifier quality include using more data or training ModernBERT-large instead of ModernBERT-base.
...
Clustering and routing decisions add negligible latency to inference. At query time, each input is assigned to the nearest centroid via a single all-MiniLM-L6-v2 embedding pass (≈\\approx14,200 sentences/s on a single GPU); the subsequent arg​min\\operatorname\*{arg\\,min} over the model pool is an in-memory lookup with negligible cost.
...
ate a statistically significant improvement of CSCR.
